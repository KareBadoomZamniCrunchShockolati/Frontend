    ...
    <div className="p-2">
      ...
      {isOpen && selectedBadge && (
        <>
          ...
          <div
            className="border-2 border-black fixed bottom-0 left-0 right-0 bg-white rounded-t-md shadow-2xl z-50 animate-slide-up px-8 h-[70vh]"
            style={{
          ...
          >
            <div className="h-2/3 flex flex-col gap-3 py-8 justify-center items-center border-3 border-primary rounded-lg mt-8 ">
              <img
                ...
                alt="selected image"
                className="h-3/4" // why height does not work in here?
              />
...
function CustomSlider({ value }: { value?: number }) {
  ...
  return (
    <div className="w-full max-w-md bg-white rounded-lg ">
      {/* Slider */}
      <div className="relative w-4/5 h-5 rounded-lg overflow-hidden m-auto border-2 border-black">
        <div
...
const Fraction = ({ up, down }: DatFractionProps) => {
  return (
    <div className="flex justify-center px-2 py-0.5 ">
      <div className=" mr-2 text-2xl">{convertToPersianDigits(String(up))}</div>
